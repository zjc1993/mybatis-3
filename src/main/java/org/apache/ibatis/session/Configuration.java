...
public class Configuration {
	...
	/**
	 * 语言驱动注册类，注册语言驱动，默认包含了2种类型的语言驱动类，分别是XMLLanguageDriver(默认值)，RawLanguageDriver
	 */
	protected final LanguageDriverRegistry languageRegistry = new LanguageDriverRegistry();
	/**
	 * 存放mapper
	 * xml中的每一个<SELECT><UPDATE><INSERT><DELETE>语句,这四个中每一个语句都对应一个MappedStatement
	 * key是namespace+标签中的ID值 value是MappedStatement对象
	 */
	...
	/**
	 * 存放mapper xml中<ResultMap>标签的信息
	 */
	protected final Map<String, ResultMap> resultMaps = new StrictMap<>("Result Maps collection");
	/**
	 * 存放mapper xml中<ParameterMap>标签的信息
	 */
	protected final Map<String, ParameterMap> parameterMaps = new StrictMap<>("Parameter Maps collection");
	/**
	 * 存放mapper xml中<KeyGenerator>标签的信息
	 */
	...
	/**
	 * 存放未处理完成的XMLStatementBuilder信息
	 * 在构建MappedStatement过程中出现了异常的时候，将把这条statementParser解析对象放到这个值中，方面随后进行处理
	 */
	protected final Collection<XMLStatementBuilder> incompleteStatements = new LinkedList<>();
	/**
	 * 存放未处理完成的CacheRefResolver信息
	 */
	protected final Collection<CacheRefResolver> incompleteCacheRefs = new LinkedList<>();
	/**
	 * 存放已经载入过的ResultMapResolver文件信息
	 */
	protected final Collection<ResultMapResolver> incompleteResultMaps = new LinkedList<>();
	/**
	 * 存放已经载入过的MethodResolver文件信息
	 */
